...
var MochaRunner, actions, altEnv, argsHint, colors, compile, deferred, doAction, env, environment, input, inspector, keypress, kids, lib, lstatSync, mocha, normalize, primaryTabComplete, program, prompt, readFileSync, readdirSync, refresh, run, secondaryTabComplete, sep, shutdown, spawn, spec, src, test, testRunner, watch, watcher, _ref, _ref1,
  __slice = [].slice;
...
if (mocha) {
  testRunner = MochaRunner.create({
    reporter: 'Dot'
  });
}

MochaRunner.on('spec_event', function(payload) {
...
test = deferred(function(_arg, file) {
  ...
  return testRunner.run([file], resolve);
});
