...
var actions, bin, colors, doAction, hint, input, inspector, keypress, kid, kids, normalize, primaryTabComplete, program, prompt, readFileSync, refresh, run, secondaryTabComplete, shutdown, spawn, webPort, _ref,
  __slice = [].slice;
...
run = function() {
  ...
  return process.stdin.on('keypress', function(chunk, key) {
    var act, action, ctrl, m, matches, meta, name, sequence, shift, _i, _len, _ref1;
    hint = '';
    try {
    ...
    if (name === 'tab') {
      try {
        _ref1 = input.match(/^(.*)\s/), m = _ref1[0], act = _ref1[1];
      } catch (_error) {}
      if (act != null) {
